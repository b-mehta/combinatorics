...
section ramsey
  ...
  local notation r`_edge_of ` X := rset' r X
  local notation `edge_of ` X := rset' 2 X
  ...
  theorem ramsey (c : finset ℕ → ℕ) (h : ∀ (e : finset ℕ), e.card = 2 → c e < 2) : ∃ (M : set ℕ), set.infinite M ∧ 
    ∃ (col : ℕ), ∀ (i ∈ (M^(2))), c i = col := 
  begin
    ...
    { obtain ⟨g, hg⟩ : {g : ℕ → { z : ℕ × set ℕ × ℕ // (∀ v ∈ z.2.1, c {z.1, v} = z.2.2) ∧ set.infinite z.2.1 ∧ z.2.2 < 2} 
                                // ∀ i j, i < j → (g i).1.1 < (g j).1.1 ∧ (g j).1.1 ∈ (g i).1.2.1 ∧ (g j).1.2.1 ⊆ (g i).1.2.1} := 
                       crec' nat.lt_wf (λ _ _ _ (x y : { z : ℕ × set ℕ × ℕ // (∀ v ∈ z.2.1, c {z.1, v} = z.2.2) ∧ set.infinite z.2.1 ∧ z.2.2 < 2}), x.1.1 < y.1.1 ∧ y.1.1 ∈ x.1.2.1 ∧ y.1.2.1 ⊆ x.1.2.1) _,
      exact ⟨λ i, (g i).1, λ i, (g i).2.1, λ i, (g i).2.2.1, λ i, (g i).2.2.2, hg⟩,
      rintros i ⟨If, IH⟩,
      cases i,
      { set v : ℕ := 0,
        have := infinite_pigeonhole (colour_neighbour_in univ v c) {i : ℕ | i < 2} (finite_lt_nat 2) _, 
          swap, rw all_colours_is_all (finite_lt_nat 2) h (mem_univ v), apply infinite_erase, apply infinite_univ_nat,
      ...
      have := infinite_pigeonhole (colour_neighbour_in (If'.1.2.1) v c) {i : ℕ | i < 2} (finite_lt_nat 2) _, 
        swap, rw all_colours_is_all (finite_lt_nat 2) h hv₁, apply infinite_erase infBi, 
      rcases classical.indefinite_description _ this with ⟨red, red_prop⟩,
      rcases classical.indefinite_description _ red_prop with ⟨red_lt_2, inf_red_edges⟩,
      refine ⟨⟨⟨v, colour_neighbour_in If'.1.2.1 v c red, red⟩, _, inf_red_edges, red_lt_2⟩, _⟩, 
        intros v hv, convert hv.2.2,
      intros j hj, 
    ...
    set a := λ i, (f i).1,
    set C := λ i, (f i).2.2,
    obtain ⟨col', collt2, inft⟩ := infinite_pigeonhole (λ col, {i : ℕ | C i = col}) {x | x < 2} (finite_lt_nat _) _,
