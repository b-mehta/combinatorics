...
-- -/
abbreviation colex_le [has_lt α] (A B : finset α) : Prop := A.to_colex ≤ B.to_colex
abbreviation colex_lt [has_lt α] (A B : finset α) : Prop := A.to_colex < B.to_colex
...
--   rcases (exists_max' (A \ B ∪ B \ A) id _) with ⟨k, hk, z⟩,
...
--   intro a, simp only [union_empty_iff, sdiff_eq_empty_iff_subset] at a,
...
--     rw [sum_union (sdiff_inter_inter _ _), sum_union (sdiff_inter_inter _ _),
