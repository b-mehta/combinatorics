...
section ramsey
  ...
  -- lemma rset'_mono {r : ℕ} {α : Type*} {m n : set α} (h : m ⊆ n) : (m^(r)) ⊆ (n^(r)) := λ x hx, ⟨hx.1, λ t ht, h (hx.2 _ ht)⟩

  -- lemma test (M : set ℕ) (e : finset ℕ) (H : e ∈ (M^(2))) : e ∈ ((@univ ℕ)^(2)) :=  ⟨H.1, λ _ _, trivial⟩

  lemma exists_mem_of_infinite (h : set α) (hi : set.infinite h) : ∃ x, x ∈ h :=
  exists_mem_of_ne_empty (λ t, hi (t.symm ▸ finite_empty))
  ...
  lemma all_colours_is_all'' {V K : set ℕ} (hk : set.finite K) {c : finset ℕ → ℕ} (h : ∀ e ∈ ((@univ ℕ)^(2)), c e ∈ K) {v : ℕ} (hv : v ∈ V) :
    bind K (colour_neighbour_in V v c) = {x ∈ V | x ≠ v} :=
  begin
    ext, simp [colour_neighbour_in, -ne.def], split, rintro ⟨_, _, xV, xv, _⟩, refine ⟨xV, xv.symm⟩,
    rintro ⟨xV, xv⟩, refine ⟨_, _, xV, xv.symm, rfl⟩, apply h, split, rw finset.card_insert_of_not_mem, rw finset.card_singleton, rwa finset.not_mem_singleton,
    intros _ _, trivial
  end
  ...
  theorem ramsey (c : finset ℕ → ℕ) (h : ∀ e ∈ rset' 2 (@univ ℕ), c e < 2) : ∃ (M : set ℕ), set.infinite M ∧ 
    ∃ (col : fin 2), ∀ (i ∈ (M^(2))), c i = col := 
  begin
    have: ∃ (a : ℕ → ℕ) (B : ℕ → set ℕ) (C : ℕ → ℕ), ∀ (i : ℕ), a (succ i) ∈ B i ∧ (∀ v ∈ B i, c {a i, v} = C i) ∧ set.infinite (B i),
      sorry,
    sorry
  end
end ramsey